...
import { configPath, type StoreConfig } from "./config.js";
...
async function records(): Promise<Preview[]> {
  try {
...
async function save(items: Preview[]) {
  await mkdir(dirname(path()), { recursive: true });
  const temp = `${path()}.${randomUUID()}.tmp`;
  await writeFile(temp, JSON.stringify(items, null, 2), { mode: 0o600 });
  await rename(temp, path());
}
export async function previewStores(): Promise<StoreConfig[]> {
  return (await records())
    ...
    .map((p) => ({
      ...
      shop: p.shop!,
      apiVersion: "2026-07",
      auth: { type: "shopify_cli" as const },
    }));
}
export async function createPreview(
  ...
  run: typeof cliJson = cliJson,
) {
  return serializeStore(path(), () =>
    withFileLock(path() + ".lock", async () => {
      const items = await records();
      const old = items.find((x) => x.requestId === a.requestId);
      if (old) {
        if (old.name !== a.name || old.country !== a.country)
          throw Error(
            "requestId already belongs to a different preview request.",
          );
        if (old.state !== "ready")
          throw Error(
            "This creation has an uncertain outcome. Use Shopify CLI store auth list to recover it before creating another.",
          );
...
export async function previewInfo(shop: string, run: typeof cliJson = cliJson) {
  ...
  server.registerTool(
    ...
    async (a) => {
      try {
        ...
        if (!s) throw Error("Unknown preview store alias.");
