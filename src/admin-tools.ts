...
export function registerAdminTools(server: McpServer) {
  function register<S extends z.ZodRawShape>(
    ...
    shape: S,
    write: boolean,
    handler: (w: Workflow, a: z.infer<z.ZodObject<S>>) => Promise<Data>,
  ) {
    server.registerTool(
      `shopify_${name}`,
      {
      ...
      async (args) => {
        ...
        try {
          ...
          const result = await handler(w, input as z.infer<z.ZodObject<S>>);
          return {
            ...textResult({
              store: w.store.alias,
              shop: w.store.shop,
  ...
  register(
    ...
    false,
    (w, a) => w.run(DOCS.products, a),
  );
  register(
    "get_product",
    "Get product details, variants and media. Follow each returned cursor independently.",
    { id: gid("Product"), ...page, mediaAfter: after },
    false,
    (w, a) => w.product(a.id, a.first, a.after, a.mediaAfter),
  );
  register(
    "search_collections",
    ...
    { query: z.string().max(1000).optional(), ...page },
    false,
    (w, a) => w.run(DOCS.collections, a),
  );
  register(
    "get_collection",
    "Get collection details, rules and a page of products.",
    { id: gid("Collection"), ...page },
    false,
    (w, a) => w.collection(a.id, a.first, a.after),
  );
  register(
    "list_orders",
  ...
  register(
    ...
    async (w, a) => {
      ...
      if (!table) throw Error("ShopifyQL returned no table.");
      const x = table.columns.findIndex((c: Data) =>
        /date|time|string/i.test(c.dataType),
      );
      const ys = table.columns.flatMap((c: Data, i: number) =>
        /money|number|integer|float|decimal|percent/i.test(c.dataType)
          ? [i]
          : [],
      );
      return {
  ...
  register(
    ...
    { id: gid("Collection"), ...collectionFields },
    true,
    async (w, a) => {
      await w.requireScopes(["write_products"]);
    ...
    async (w, a) => {
      ...
      if (a.collectionId) {
        ...
        do {
          ...
          rows.push(...d.collection.products.nodes.map((p: Data) => p.id));
  ...
  register(
    ...
    async (w, a) => {
      ...
      if (!d.node) throw Error("Bulk operation not found.");
      return {
        ...d.node,
