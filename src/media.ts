...
import { type Workflow, type Data, WorkflowError } from "./admin-workflows.js";
...
export async function uploadImage(
  ...
  if (a.imageFile) {
    ...
    try {
      ...
      if (bytes.length > 20 * 1024 * 1024) throw Error("Image exceeds 20 MiB.");
    } finally {
      await file.close();
    }
    ...
    if (!target) throw Error("Shopify did not return an upload target.");
    const url = new URL(target.url);
    // Only the storage hosts used by Shopify staged uploads receive local bytes.
    if (
      url.protocol !== "https:" ||
      !/(^|\.)shopify\.com$|(^|\.)shopifycloud\.com$|(^|\.)googleapis\.com$|(^|\.)amazonaws\.com$/.test(
        url.hostname,
      )
    )
  ...
  if (!id) throw Error("Shopify did not return a file ID.");
  for (let i = 0; i < 10; i++) {
