...
10. For a new custom operation, read [references/shopify-admin-companion.md](references/shopify-admin-companion.md) before you write GraphQL.
11. Use `shopify_graphql_mutation` only after the user authorizes the exact store and change.
12. Set `confirm` to `true` only when that authorization exists.
13. Use cursor pagination and request only necessary fields.
14. Preserve Shopify GraphQL user errors and per-store partial failures in the response.
