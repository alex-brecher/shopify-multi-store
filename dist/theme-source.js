...
export async function dawnSource() {
    const path = join(dirname(configPath()), "theme-sources", `dawn-${DAWN_COMMIT}`);
    ...
    return withFileLock(path + ".lock", async () => {
        ...
        try {
            ...
            throw Error("Could not fetch the pinned Shopify Dawn source. Git and GitHub access are required.");
        }
    });
}
