...
export async function validateDocument(document, version) {
    const schema = await adminSchema(version);
    try {
        return validate(schema, parse(document, { maxTokens: 15_000 })).map((e) => ({ message: e.message, locations: e.locations }));
    }
    catch (error) {
        return [
...
export async function inspectType(name, version) {
    const schema = await adminSchema(version);
    const type = schema.getType(name);
    if (!type)
        throw new Error(`Unknown GraphQL type ${name} in ${version}.`);
    return {
        apiVersion: version,
        name,
