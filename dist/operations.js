...
export function operation(document, expected) {
    ...
    if (selected.operation === "subscription" ||
    ...
    if (ast.definitions.some((d) => d.kind !== Kind.OPERATION_DEFINITION &&
...
export function mutationErrors(document, data) {
    ...
    const errors = [];
    function walk(set, value, path, active = new Set()) {
        ...
        for (const field of set.selections) {
            if (field.kind === Kind.FRAGMENT_SPREAD) {
                ...
                if (fragment)
                    walk(fragment.selectionSet, value, path, new Set([...active, field.name.value]));
            }
                ...
                if (/^(userErrors|mediaUserErrors)$/.test(field.name.value) &&
                    Array.isArray(child)) {
                    errors.push(...child.map((error) => ({ path: [...path, key], error })));
                }
