...
export async function getProductEverywhere(aliases, identifier, matchBy) {
    ...
    const stores = Object.fromEntries(report.results.map((result) => {
        if (!result.ok)
            return [result.store, { ok: false, error: result.error, matches: [] }];
        if (matchBy === "handle") {
...
export async function searchProductsMany(aliases, query, first) {
...
export async function fulfillmentSlaReport(aliases, lookbackDays, slaDays, first) {
    ...
    const summaries = report.results.map((result) => {
        ...
        const numericAges = orders.map((order) => order.ageHours).filter((age) => typeof age === "number");
        const pageInfo = recordValue(connectionRecordFrom(result, "orders").pageInfo);
        return {
            store: result.store,
            ok: result.ok,
...
export async function catalogGapReport(aliases, first) {
    ...
    const gaps = [...handles].sort().map((handle) => {
        const stores = Object.fromEntries(report.results.map((result) => {
            if (!result.ok)
                return [result.store, { available: false, unknown: true, error: result.error }];
            const product = nodesFrom(result, "products").find((node) => String(node.handle ?? "").toLowerCase() === handle);
