...
const PreviewProduct = z
    ...
    .strict();
export async function seedPreviewProducts(alias, products) {
    ...
    if (!online)
        throw Error("Online Store publication is unavailable.");
    const ids = [];
    for (const p of products) {
        ...
        if (!id)
            throw Error("No preview product ID returned.");
        ids.push(id);
...
const escape = (s) => s
    ...
    .replaceAll(">", "&gt;")
    .replaceAll('"', "&quot;");
export async function buildDesign(design, sourceDirectory) {
    ...
    const settings = JSON.parse(await readFile(join(path, "config/settings_data.json"), "utf8"));
    const current = settings.presets.Dawn;
    for (const scheme of Object.values(current.color_schemes))
    ...
    await writeFile(join(path, "config/settings_data.json"), JSON.stringify(settings));
    const index = JSON.parse(await readFile(join(path, "templates/index.json"), "utf8"));
    ...
    if (design.layout === "products-first")
        index.order.reverse();
    ...
    index.sections = Object.fromEntries(index.order.map((k) => [k, index.sections[k]]));
    await writeFile(join(path, "templates/index.json"), JSON.stringify(index));
    return path;
}
async function resultLinks(job) {
    return {
        status: "complete",
        temporary: true,
        storefrontPreviews: await Promise.all(job.themes.map(async (t) => ({
            ...t,
            ...(await previewInfo(t.shop)),
        }))),
        notice: "Each design is a separate temporary Shopify store. Claim the design you want to keep.",
    };
}
export function registerAsyncPreview(server, name, definition, handler) {
    const active = new Set();
    const statusPath = (id) => join(dirname(configPath()), `preview-design-${id}.status.json`);
    const pending = (id) => textResult({
        status: "pending",
        requestId: id,
        notice: "Storefront previews are being built. Check status with shopify_get_new_store_preview_status.",
    });
    const readStatus = async (id) => {
        const status = JSON.parse(await readFile(statusPath(id), "utf8"));
        if (status.state === "complete") {
            const job = JSON.parse(await readFile(join(dirname(configPath()), `preview-design-${id}.json`), "utf8"));
            return textResult(await resultLinks(job));
        }
        if (status.state === "failed")
            return textResult(status.error, true);
        return active.has(id)
            ? pending(id)
            : textResult({
                error: "Preview generation was interrupted. Inspect the creation receipts before recovery.",
                requestId: id,
            }, true);
    };
    server.registerTool(name, definition, async (args) => {
        ...
        try {
            const fingerprint = createHash("sha256")
                ...
                .digest("hex");
            try {
                const existing = JSON.parse(await readFile(statusPath(a.requestId), "utf8"));
    ...
    server.registerTool("shopify_get_new_store_preview_status", {
        description: "Check an asynchronous storefront preview request and retrieve fresh preview and claim links.",
        _meta: UI_META,
...
export function registerPreviewDesignTools(server) {
    registerAsyncPreview(server, "shopify_get_new_store_previews", {
        description: "Start an asynchronous build of 1–3 designed storefront previews on separate NEW temporary Shopify stores, with a real claim link. Generate concrete design specifications from the user brief. Uses Shopify Dawn; does not alter an existing store. Reuse requestId after interruption. Generate demo products and designs from the brief. Products, pricing, and storefront copy remain editable after claiming.",
        _meta: UI_META,
        inputSchema: z
            .object({
            ...
            products: z
                ...
                .min(1)
                .max(10)
                .describe("Generate demo product concepts and proposed prices from the brief. Do not claim these are verified supplier offers."),
        })
            .strict(),
        annotations: {
            readOnlyHint: false,
        ...
        const file = join(dirname(configPath()), `preview-design-${a.requestId}.json`);
        return serializeStore(file, () => withFileLock(file + ".lock", async () => {
            try {
                let job;
                try {
                ...
                if (job) {
                    ...
                    if (job.state !== "complete")
                        return textResult({
                            error: "Preview design has an incomplete or uncertain outcome. Inspect existing themes before retrying.",
                            ...job,
                        }, true);
                    return textResult(await resultLinks(job));
                }
                ...
                for (const [i, design] of a.designs.entries()) {
                    const hash = createHash("sha256")
                        ...
                        .digest("hex")
                        .slice(0, 32);
                    const childId = hash.replace(/(.{8})(.{4})(.{4})(.{4})(.{12})/, "$1-$2-$3-$4-$5");
                    const target = i === 0
                        ? store
                        : await createPreview({
                            requestId: childId,
                    ...
                    const productIds = await seedPreviewProducts(target.alias, a.products);
                    job.products = {
                        ...(job.products ?? {}),
                    ...
                    await w.run("mutation PublishPreview($id:ID!){themePublish(id:$id){theme{id name role} userErrors{field message}}}", { id: `gid://shopify/OnlineStoreTheme/${r.theme.id}` });
                    const verified = await w.run("query VerifyPreview($id:ID!){theme(id:$id){id role}}", { id: `gid://shopify/OnlineStoreTheme/${r.theme.id}` });
...
//# sourceMappingURL=preview-designs.js.map